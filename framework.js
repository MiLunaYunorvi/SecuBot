...
function Framework(options) {
   ...
   * @property {string} [membershipRulesAllowedResponse] - Message from bot when it detects that an the memberships of a space it is in have changed in
        ...
        No message will be sent if this is set to an empty string.
   * @property {Boolean} [fedramp] - If specified, enables the framework to support the Webex FedRAMP environment.
   * 
   */
...
Framework.prototype.showHelp = function (header, footer,footer2) {
  ...
  const helpObject = {
    '- (S) Soluciones:':'Encuentra recursos de ayuda rápida para cada solución', 
    '- (A) Ataques💣:': 'Conoce que soluciones son de ayuda ante ataques específicos ', 
    '- Funcion Beta': 'Prueba una conversación libre, por ejemplo: "¿Cómo Umbrella previene el pishing?"'}
  Object.entries(helpObject).map(([key, value]) => {
...
Framework.prototype.onMessageCreated = function (message) {
  this.debug(`Processing message "${message.text}" in room ID: ${message.roomId}`);
    ...
    return this.getTrigger('message', message)
      .then(trigger => {
        ...
        if (trigger.text) {
          ...
          this.debug(`Evalauting message against ${this.lexicon.length + 1} registered framework.hears() handlers...`);
          var matched = _.filter(this.lexicon, lex => {

            this.debug(`Evalauting message against phrase: ${lex.phrase}`)
            // if lex.phrase is regex
            if (lex.phrase && lex.phrase instanceof RegExp && lex.phrase.test(trigger.text)) {
              this.debug('Found a regex match.');
              return true;
            }
            ...
            // if lex.phrase is string and this is NOT a bot account
              ...
              this.debug('Found a match without checking for mentions (non bot accounts only)');
              return true;
            }
            ...
            // if lex.phrase is string and this is a bot account
              ...
              if (regexPhrase.test(trigger.text)) {
                this.debug('Found a string match and bot was mentioned!');
                return true;
              }
              return false;
            }
        ...
        // else trigger.text not present...
          ...
          this.debug('No trigger.text to evalaute - this is unexpected, please report this to Webex Developer support!');
          return when(false);
        }

        // if matched
        if (matched.length && typeof this.authorize === 'function') {
          this.debug(`Found ${matched.length} handlers.  Checking registerd authorization function...`);
        ...
        // else, if matched and no authorization configured, run command
          ...
          this.debug(`Found ${matched.length} handlers.  Calling framework.hears() handlers...`);
          runActions(matched, bot, trigger, this);
          return when(trigger);
        }
        ...
        // else, do nothing...
          ...
          this.debug(`(No Hears Called!) Does your app need a catch-all handler?`);
